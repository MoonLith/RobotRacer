"""Programme gerant les deplacements du Robot apres l'analyse d'image"""
...
class Robot :
    ...
    def __init__(self, vitesse=1500) :
        ...
        self.roues_droites = int((self.roues_a_droite_max
            -self.roues_a_gauche_max)/2)
    ...
    def virage_gauche(self,temps,temps2):
        #etape une arrière droite
        GPIO.output(13, GPIO.LOW)
        GPIO.output(12, GPIO.LOW)
        ...
        time.sleep(temps)
        #étape 2
        GPIO.output(11, GPIO.LOW)#Arret
        GPIO.output(15, GPIO.LOW)
        GPIO.output(13, GPIO.LOW)#Forward
        GPIO.output(12, GPIO.LOW)
        time.sleep(temps2)
    def virage_droite(self,temps,temps2):
        #etape une arrière droite
        GPIO.output(13, GPIO.LOW)
        GPIO.output(12, GPIO.LOW)
    ...
    def tourner(self, centre_x) :
        """Cette fonction tourne les roues du Robot.
        Elle est appelee avec un parametre :
        - centre_x : la valeur de l'abscisse du centre de la forme percue sur l'image
        
        Une excpeption de type ValueError est levee si la valeur de centre_x
        ...
        if (centre_x > self.borne_droit_max) :
            if (centre_x >= self.borne_droite) :
                ...
                self.pwm.set_pwm(Robot.ROUE, 0, self.virage_droite_max)
            else :
                ...
                inclinaison = int((taille_interv_roues)*(((self.borne_droite)
                    -centre_x)//(taille_interv_centre_x))+self.roues_droites)
        ...
        # Virage a gauche
            ...
            if (centre_x <= self.borne_gauche) :
                ...
                # Braquage des roues adapte
                inclinaison = int((taille_interv_roues)*(((self.borne_droit_min)
                    -centre_x)//(taille_interv_centre_x))+self.roues_a_gauche_max)
