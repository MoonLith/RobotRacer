...
def installfile(IP,NOM_USER,MDP):
    ...
    elle verifiera via un fichier externe : checker.txt
    pour s'arrurer que l'utilisateur ne réinstalle pas à chaque utilisation
    les fichiers d'utilisations du robot racer
    """
    ...
    filelistracer = ['racer_calculs_valeurs.py','racer_image.py','racer_main.py','racer_mouvement.py','racer_arret.py']
    ...
    try :
        if file.mode == 'r+':#on s'assure que le fichier soit en mode lecture et ecriture
            ...
            content = file.readline()#seul la première ligne nous intéresse
            print(content)


            if not content.strip(): #si il n'y a pas de contenu dans le fichier

                print("ok")
                user.connect(IP,port=22 ,username=NOM_USER, password=MDP)  # MODIFIER LE PORT
                ...
                user.connect(IP,port= 22, username=NOM_USER, password=MDP)  # MODIFER LE PORT
                stdin, stdout, stderr = user.exec_command('ls robotRacerOff/') #on creer un dossier
                out = stdout.readlines()
                print('\n'.join(out))

    except FileNotFoundError as FileError:
        #En cas d'érreur, on l'indique à l'utlisateur
