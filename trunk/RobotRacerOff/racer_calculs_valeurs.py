...
le braquage de roues avant a adopter pour tous les virages :
...
- int:vitesseMoteurs(int:type_virage, int:vitesse) : fonction qui a partir d'un
type de virage (donne par la fonction typeDeVirage()) et de la vitesse que
prend le Robot dans les lignes droites, renvoie la vitesse que doivent prendre
les moteurs a cet instant. L'entier renvoye est donne en parametre a la fontion
qui gere les servos avant dans racer_mouvement.py.

- int,int:valPourMouvement(int:position_x, int:vitesse) : fonction qui donne au
module main les valeurs qu'il enverra a l'objet 'robot' comme le braquage des
roues et la vitesse des moteurs, en appelant differentes fonctions du module.
Elle est appelee dans main avec la position absolue de la forme et la vitesse
en ligne droite du Robot.
"""

# On definie des valeurs significatives dans le domaine de definition de la
# variable position_x, qui dependent de la resolution de la camera
width = 192
...
def centrexCamera(centre_x, camera_x) :
    ...
    de la camera, c'est-a-dire en quelque sorte : remettre centre_x dans l'axe
    initial du Robot.

    Prend deux parametres :
    - int:centre_x : l'abscisse du centre geometrique de la forme percue par la
    camera.
    ...
    position_x = int(centre_x-2.3*(camera_x-250))

    return position_x


def valPourCamera(position_x) :
    """Cette fonction donne la position que doit prendre la camera selon la
...
def typeDeVirage(position_x) :
    """Fonction qui, a partir du placement de la forme par rapport au Robot,
    renvoie le type de virage dans lequel se trouve le Robot.

    Prend un parametre :
    - int:position_x : entier resultant de la fonction centrexCamera.

    Renvoie une valeur :
    - int:type_virage : une constante qui correspond a un type de virage :
    ...
    if (position_x <= borne_droit_max) and (position_x >= borne_droit_min) :
        print("Va tout droit")
        return EN_FACE

    # Virage a droite
    elif (position_x > borne_droit_max) :
        if (position_x >= borne_droite) :
            print("Virage serre a droite")
...
def inclinaisonRoues(type_virage, position_x) :
    ...
    Une exception de type ValueError est lancee si type_virage n'est pas
    compris entre -2 et 2.
    """

    # On definie les bornes des valeurs a envoyer a set_pwm pour tourner les
    # roues dans la methode tourner() de la classe Robot (module mouvement)
    roues_a_droite_max = 275
    roues_a_gauche_max = 185
    roues_droites = int((roues_a_droite_max+roues_a_gauche_max)/2)

    if type_virage == EN_FACE :
        return roues_droites

    elif type_virage == A_DROITE_SERRE:
        ...
        inclinaison = int((taille_interv_roues)*(0.5-((borne_droite-position_x)/(taille_interv_centre_x)))+roues_droites)
        return inclinaison
        ...
        # Braquage des roues adapte
        inclinaison = int((taille_interv_roues)*(0.5-((borne_droit_min-position_x)/(taille_interv_centre_x)))+roues_a_gauche_max)
        return inclinaison
...
def vitesseMoteurs(type_virage, vitesse) :
    ...
    Renvoie une valeur :
    - int:vitesse_ideale : un entier correspondant a la valeur que prendront
    les fonctions qui configurent la vitesse des moteurs.

    Une exception de type ValueError est lancee si le typt_virage n'est pas
    compris entre -2 et 2.
    """
    ...
    elif type_virage == A_DROITE_SERRE or type_virage == A_GAUCHE_SERRE :
...
def valPourMouvements(position_x, vitesse) :
    ...
    les valeurs utilisees par la methode robot.tourner() de racer_mouvement.py.

    Prend deux parametres :
    - int:position_x : represente la position du circuit par rapport au robot.
    - int:vitesse : la vitesse du Robot pour une trajectoire rectiligne.
