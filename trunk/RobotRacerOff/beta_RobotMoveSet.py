...
def man_droite(coord_right_x,coord_right_y,coord_top_x,coord_top_y,width,height) :
    ...
    if coord_right_x > int(width * 60 / 100) and coord_right_y > int(height* 10/100) and coord_top_y > int(height* 20/100):
        pwm.set_pwm(cam_x, 0, cam_droite)  # vitesse

        tourner_droite(1)

        print("VIRAGE DROITE TYPE 1")
        #virage_droite(0.95,1)
...
def man_gauche(coord_left_x,coord_left_y,coord_top_x,coord_top_y,width,height):
    ...
    if coord_left_x < int(width * 15 / 100) and coord_left_y > int(height * 0/ 100)  \
            and coord_left_y < int(height *45/100) and coord_top_x < int(
            width * 15 / 100) and coord_top_y < int(height *0/100):
        print("VIRAGE GAUCHE")
        tourner_gauche(1)
