...
import install
from stream_thread import InitClientStream
...
def sshconnexion(IP,NOM_USER,MDP):
    ...
    user.connect(IP,port=22, username=NOM_USER, password=MDP)#on se connecte par SSH
...
def onclick(event):
    """Fonction permettant de gerer les evenements cliqué de la page autonome du robot racer """
    ...
    if event == 'racer': #le bouton cliqué correspond à celui du mode racer
        ...
        client_stream = InitClientStream(IP)
        client_stream.start()
        client_stream.join()
        out = stdout.readlines(1024)
        print('\n'.join(out))
